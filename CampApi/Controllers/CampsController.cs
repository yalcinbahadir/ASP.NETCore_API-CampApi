...
namespace CampApi.Controllers
{
    ...
    public class CampsController : ControllerBase
    {
        ...
        [MapToApiVersion("1.0")]
        public async Task<ActionResult<CampModel>> GetCamp10(string moniker)
        {
            try
            {
        ...
        public async Task<ActionResult<CampModel>> GetCamp11(string moniker)
        {
            try
            {
                var result = await _repository.GetCampAsync(moniker, true); //Camp
                if (result == null)
                {
                    return NotFound();
                }
        ...
        public async Task<ActionResult<Camp>> Post(CampModel model)
        {
            try
            {
                ...
                if (result)
                {
                    return CreatedAtAction(nameof(GetCamp11), new { Moniker = camp.Moniker }, camp);
                }
                return BadRequest();
            }
            catch (Exception ex)
